...
import {
    organizeImportsCommands,
    provideOrganizeImports,
} from "./organize-imports.js";
import {
    CommandTypes,
    EventName,
    OrganizeImportsMode,
...
} from "./document-symbol.js";
import {
    toSyntaxServerConfiguration,
    TsServerLogLevel,
...
} from "./features/call-hierarchy.js";
...
export class LspServer {
    ...
    private get tspClient(): TspClient {
        if (!this._tspClient) {
            throw new Error(
                'TS client not created. Did you forget to send the "initialize" request?'
            );
        }
    ...
    async initialize(
        params: TypeScriptInitializeParams
    ): Promise<lsp.InitializeResult> {
        this.logger.log("initialize", params);
        if (this._tspClient) {
            throw new Error(
                'The "initialize" request has already called before.'
            );
        }
        ...
        const userInitializationOptions: TypeScriptInitializationOptions =
        ...
        if (textDocument) {
            const { codeAction, completion, definition, publishDiagnostics } =
                textDocument;
            if (codeAction) {
            ...
            if (completion) {
                ...
                if (completionItem) {
                    ...
                    this.features.completionLabelDetails =
                        this.configurationManager.tsPreferences
            ...
            if (definition) {
                this.features.definitionLinkSupport =
                    definition.linkSupport &&
                    typescriptVersion.version?.gte(API.v270);
            }
        ...
        this._tspClient = new TspClient({
            ...
            logVerbosity:
                tsserverLogVerbosity ?? this.options.tsserverLogVerbosity,
            disableAutomaticTypingAcquisition,
            maxTsServerMemory,
            npmLocation,
            locale,
        ...
        this.diagnosticQueue = new DiagnosticEventQueue(
            (diagnostics) =>
                this.options.lspClient.publishDiagnostics(diagnostics),
            this.documents,
            this.features,
            this.logger,
            this._tspClient
        ...
        this.configurationManager.setAndConfigureTspClient(
            this.workspaceRoot,
            this._tspClient,
            hostInfo
        );
        ...
        const prepareSupport =
            ...
            this.tspClient.apiVersion.gte(API.v310);
        const initializeResult: lsp.InitializeResult = {
            capabilities: {
                ...
                codeActionProvider: clientCapabilities.textDocument?.codeAction
                    ...
                    ? {
                          codeActionKinds: [
                              ...TypeScriptAutoFixProvider.kinds.map(
                                  (kind) => kind.value
                              ),
    ...
    private findTypescriptVersion(
        userTsserverPath: string | undefined
    ): TypeScriptVersion | null {
        const typescriptVersionProvider = new TypeScriptVersionProvider(
            userTsserverPath || this.options.tsserverPath,
            this.logger
        );
        ...
        if (this.workspaceRoot) {
            const workspaceVersion =
                typescriptVersionProvider.getWorkspaceVersion([
                    this.workspaceRoot,
                ]);
            if (workspaceVersion) {
        ...
        const args: ts.server.protocol.SetCompilerOptionsForInferredProjectsArgs =
            {
                options: {
                    ...getInferredProjectCompilerOptions(
                        this.configurationManager.workspaceConfiguration
    ...
    didChangeConfiguration(params: lsp.DidChangeConfigurationParams): void {
        this.configurationManager.setWorkspaceConfiguration(
            params.settings || {}
        );
        this.setCompilerOptionsForInferredProjects();
        const ignoredDiagnosticCodes =
            this.configurationManager.workspaceConfiguration.diagnostics
                ?.ignoredCodes || [];
        this.diagnosticQueue?.updateIgnoredDiagnosticCodes(
    ...
    readonly doRequestDiagnosticsDebounced = debounce(
        () => this.doRequestDiagnostics(),
        200
    );
    protected async doRequestDiagnostics(): Promise<void> {
        ...
        try {
            return await this.tspClient.requestGeterr(
                { delay: 0, files },
    ...
    didOpenTextDocument(params: lsp.DidOpenTextDocumentParams): void {
        ...
        if (this.documents.open(file, params.textDocument)) {
            this.tspClient.notify(CommandTypes.Open, {
                ...
                scriptKindName: this.getScriptKindName(
    ...
    didChangeTextDocument(params: lsp.DidChangeTextDocumentParams): void {
        ...
        if (!document) {
            this.logger.error(
                `Received change on non-opened document ${textDocument.uri}`
            );
            throw new Error(
                `Received change on non-opened document ${textDocument.uri}`
            );
    ...
    private async getSymbolLocations(
        ...
        return response.body.map((fileSpan) =>
            toLocation(fileSpan, this.documents)
        );
    }
    ...
    async completion(
        ...
        token?: lsp.CancellationToken
    ): Promise<lsp.CompletionList | null> {
        const file = uriToPath(params.textDocument.uri);
        ...
        if (!document) {
            throw new Error(
                `The document should be opened for completion, file: ${file}`
            );
        }

        const completionOptions =
            this.configurationManager.workspaceConfiguration.completions || {};

        const response = await this.interuptDiagnostics(() =>
            this.tspClient.request(
        ...
        if (isMemberCompletion) {
            const dotMatch =
                ...
                undefined;
            if (dotMatch) {
                const startPosition = lsp.Position.create(
    ...
    async completionResolve(
        ...
        const document = item.data?.file
            ? this.documents.get(item.data.file)
            : undefined;
    ...
    async hover(
        ...
        const result = await this.interuptDiagnostics(() =>
            this.getQuickInfo(file, params.position, token)
        );
        if (!result?.body) {
            return { contents: [] };
        }
        ...
        Previewer.addMarkdownDocumentation(
            contents,
            documentation,
            tags,
            this.documents
    ...
    > {
        ...
        const response = await this.tspClient.request(
            CommandTypes.Rename,
            Position.toFileLocationRequestArgs(file, params.position),
            token
        );
    ...
    async rename(
        ...
        token?: lsp.CancellationToken
    ): Promise<lsp.WorkspaceEdit | undefined | null> {
        const file = uriToPath(params.textDocument.uri);
    ...
    async documentFormatting(
        ...
        if (!document) {
            throw new Error(
                `The document should be opened for formatting', file: ${file}`
            );
        }
    ...
    async signatureHelp(
        ...
        const response = await this.interuptDiagnostics(() =>
            this.getSignatureHelp(file, params, token)
        );
        if (!response?.body) {
            return undefined;
        }
    ...
    async codeAction(
        ...
        const kinds = params.context.only?.map(
        ...
        for (const kind of kinds || []) {
            for (const command of organizeImportsCommands) {
                if (
                    ...
                    (command.minVersion &&
                        this.tspClient.apiVersion.lt(command.minVersion))
                ) {
                    continue;
                }
        ...
        if (kinds && !this.pendingDebouncedRequest) {
            const diagnostics =
                this.diagnosticQueue?.getDiagnosticsForFile(file) || [];
    ...
    protected async getCodeFixes(
        fileRangeArgs: ts.server.protocol.FileRangeRequestArgs,
        context: lsp.CodeActionContext,
        token?: lsp.CancellationToken
    ): Promise<ts.server.protocol.CodeFixResponse | undefined> {
    ...
    protected async getRefactors(
        ...
        context: lsp.CodeActionContext,
        token?: lsp.CancellationToken
    ): Promise<ts.server.protocol.GetApplicableRefactorsResponse | undefined> {
    ...
    async executeCommand(
        ...
        if (arg.command === Commands.APPLY_WORKSPACE_EDIT && arg.arguments) {
            ...
            arg.command === Commands.APPLY_CODE_ACTION &&
            arg.arguments
        ) {
            const codeAction = arg
                .arguments[0] as ts.server.protocol.CodeAction;
            ...
            arg.command === Commands.APPLY_REFACTORING &&
            arg.arguments
        ) {
            const args = arg
                .arguments[0] as ts.server.protocol.GetEditsForRefactorRequestArgs;
            const response = await this.tspClient.request(
            ...
            const [pluginName, configuration] = arg.arguments as [
                string,
                unknown
            ];
            ...
            arg.command === Commands.APPLY_RENAME_FILE &&
            arg.arguments
        ) {
            ...
            arg.command === Commands.APPLY_COMPLETION_CODE_ACTION &&
            arg.arguments
        ) {
            const [_, codeActions] = arg.arguments as [
                string,
                ts.server.protocol.CodeAction[]
            ];
            ...
            const [uri, position] = (arg.arguments || []) as [
                lsp.DocumentUri?,
                lsp.Position?
            ];
            const reporter =
                await this.options.lspClient.createProgressReporter(
                    token,
                    workDoneProgress
                );
    ...
    protected async applyFileCodeEdits(
        edits: ReadonlyArray<ts.server.protocol.FileCodeEdits>
    ): Promise<boolean> {
        if (!edits.length) {
            return false;
        }
        ...
        for (const edit of edits) {
            changes[pathToUri(edit.fileName, this.documents)] =
    ...
    protected async applyRenameFile(
        sourceUri: string,
        targetUri: string,
        token?: lsp.CancellationToken
    ): Promise<void> {
    ...
    protected async getEditsForFileRename(
        ...
        targetUri: string,
        token?: lsp.CancellationToken
    ): Promise<ReadonlyArray<ts.server.protocol.FileCodeEdits>> {
    ...
    async documentHighlight(
        arg: lsp.TextDocumentPositionParams,
        token?: lsp.CancellationToken
    ...
    async foldingRanges(
        ...
        if (!document) {
            throw new Error(
                `The document should be opened for foldingRanges', file: ${file}`
            );
        }
        const response = await this.tspClient.request(
    ...
    protected asFoldingRange(
        span: ts.server.protocol.OutliningSpan,
    ...
    protected async onTsEvent(event: ts.server.protocol.Event): Promise<void> {
        if (
            event.event === EventName.semanticDiag ||
            event.event === EventName.syntaxDiag ||
            event.event === EventName.suggestionDiag
        ) {
    ...
    async inlayHints(
        ...
        token?: lsp.CancellationToken
    ): Promise<lsp.InlayHint[] | undefined> {
    ...
    async linkedEditingRange(
        params: lsp.LinkedEditingRangeParams,
        token?: lsp.CancellationToken
    ): Promise<lsp.LinkedEditingRanges | null> {
        const file = uriToPath(params.textDocument.uri);
        if (!file) {
            return null;
        }
    ...
    async getSemanticTokens(
        ...
        return {
            data: SemanticTokens.transformSpans(doc, response.body.spans),
        };
    }
}
